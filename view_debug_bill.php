                ...
                <div style=" float:left; width:98%; min-height:800px; margin-top:0px; padding:15px 1%; background:#ffffff; overflow:hidden; display:block">
                    ...
                    <div style="float:left; width:100%; margin-top:0px; overflow:hidden; display:block">
                        <div style="float:left">
                            ...
                            <input class="iinput name_imme ino_ime_input" type="text" name="seller_name" placeholder="请输入卖家拼音首字母" autocomplete="off"/>
...
while($row_bill=$p[0]->fetch(PDO::FETCH_ASSOC))
{
    ...
    $tp=rselect("*","ydf_bank",array("bank_id=?",$row_bill["bill_bank_id"]));
    $bank_type=0;
    if ($trow=$tp->fetch())
        $bank_type=$trow["bank_type"];
?>
                        <div style="position:relative;width:100%;<?php echo $color?> padding:10px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block">
                            ...
                            <div style="float:left;width:8%; height:25px; padding:10px 0; text-align:center"><?php echo $row_bill["bill_bank_id"];echo "($bank_type)"; ?></div>
...
<?php 
    echo "close:{$row_bill['bill_is_close']};";
