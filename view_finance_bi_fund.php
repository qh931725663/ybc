...
<div class="search_box">
    <div id="real_time_profit">
        <div style="width:25%; float:left; text-align:center;">
            ...
            <p style="font-size:16px; padding-top:10px;" id="zijinzonge">
            <?php
                include_once "{$root_path}/model/model_bi.php";
                $p=rselect("*","ydf_bank",array("bank_boss_id=? and bank_type='4'",$_SESSION["ERP_ACCOUNT_USER_BOSS_M_BIANHAO"]));
                if($row=$p->fetch()){
                }
                list($historys,$addup_a) = get_history_bank_fund($ymd="day",$row["bank_id"]);
                echo sum_addup($addup_a);
            ?>
            </p>
        </div>
        <div style="width:24%; float:left; text-align:center;">
            <p style="font-size:14px; padding-top:25px;">当前应收货款</p>
            <p style="font-size:16px; padding-top:10px;" id="yingshouhuokuan">
                <?php
                include_once "{$root_path}/model/model_bi.php";
                list($historys,$addup_b)= get_history_sellers($ymd="day",null);
                echo sum_addup($addup_b);
                ?>
            </p>
        </div>
        <div style="width:24%; float:left; text-align:center;">
            <p style="font-size:14px; padding-top:25px;">代销可提现</p>
            <p style="font-size:16px; padding-top:10px;" id="daixiaotixian">
            <?php
                include_once "{$root_path}/model/model_bi.php";
                $historys=array();
                $index=array();
                $addup_c=array();
                get_history_agent_pools($ymd="day",null,$historys,$index,$addup_c);
                echo sum_addup($addup_c);
            ?>
            </p>
        </div>
        <div style="width:24%; float:left; text-align:center;">
            <p style="font-size:14px; padding-top:25px;">经销应付货款</p>
            <p style="font-size:16px; padding-top:10px;" id="jingxiaohuokuan">
                <?php
                   include_once "{$root_path}/model/model_bi.php";
                   list($historys,$addup_d)= get_history_dealer_gcyf($ymd="day",null);
                   echo sum_addup($addup_d);
                ?>
            </p>
        </div>
    </div>
...
<!-- refresh_begin -->
<span id="zjze" style="display:none"><?php echo sum_addup($addup_a); ?></span>
<span id="yshk" style="display:none"><?php echo sum_addup($addup_b); ?></span>
<span id="dxtx" style="display:none"><?php echo sum_addup($addup_c); ?></span>
<span id="jxhk" style="display:none"><?php echo sum_addup($addup_d); ?></span>
...
<script>/*n*/
    $("#zijinzonge").html($("#zjze").html());
    $("#yingshouhuokuan").html($("#yshk").html());
    $("#daixiaotixian").html($("#dxtx").html());
    $("#jingxiaohuokuan").html($("#jxhk").html());
