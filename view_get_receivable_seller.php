...
$receivable_fund=$_REQUEST["var_receivable_fund"];
?>
                        <form id="vform_receivable_fund">
                        <div style="float:left; width:100%; line-height:1.8; overflow:hidden; display:block">
                            <p style="float:left; width:100%; padding:5px 0; display:block">
                                <span style="float:left; width:100px; margin:5px 0; color:#999999; text-align:right">卖家：</span>
                                <span style="float:left; margin:5px 0"><?php echo $rowseller["seller_name"] ?></span>
                            </p>
                            <p style="float:left; width:100%; padding:5px 0; display:block">
                                <span style="float:left; width:100px; margin:5px 0; color:#999999; text-align:right">应收资金：</span>
                                <span style="float:left; margin:5px 0"><?php echo $receivable_fund ?></span>
                            </p>
                            <p style="float:left; width:100%; padding:5px 0; display:block">
                            ...
                            <p style="float:left; width:100%; padding:5px 0; display:block">
                                <span style="float:left; width:100px; color:#999999; margin:5px 0; text-align:right"><span style="color:red">*</span> 实收金额：</span>
                                <span style="float:left;">
                        ...
                        <div style="float:left; width:100%; margin-top:10px; overflow:hidden; display:block">
                            <span id="tip_notice_receivable_seller" style="float:left; margin-left:100px"></span>
                        </div>
                        <div style="float:left; width:100%; margin-top:10px; overflow:hidden; display:block">
                            <span id="receivable_seller_affirm_btn" onClick="/**/PostFactoryPay()" style="float:left; margin-left:100px; margin-bottom:50px; padding:7px 20px; background:#ee583d; color:#FFFFFF; cursor:pointer">确认已支付</span>
                        </div>
                        <input type="hidden" name="table[1][bill_seller]" value="<?php echo $_REQUEST["var_seller_id"]?>" />
                        <input type="hidden" name="table[1][bill_desc]" value="" />
                        <input type="hidden" name="table[1][bill_type]" value="mjss" />
                        </form>
                        <script>
                            $("#vform_receivable_fund").on('keydown',function(e){
                                if(e.keyCode == 13){
                                    //模拟点击登陆按钮，触发上面的 Click 事件
                                    $('#vform_receivable_fund input,select').blur();
                                    $("#receivable_seller_affirm_btn").click();
                                }
                            });
                        </script>