...
debug($total_kucun);
//得到货款总额
$total_payment=sum_addup($addup,"pool_fund");//取货款总计
...
for ($i=$offset;$i<$offset+$pagesize && $i<$rowcount;$i++)
{
    ...
    for($j=0;$j<count($dangkou_arr);$j++){
        ...
        if(array_key_exists($ro["detail_p_huohao"].'#'.$huo_dang, $detail_addup)==false){
            ...
            $total_inventory=$total_inventory+floatval($detail_addup[$ro["detail_p_huohao"].'#'.$huo_dang]['pool']);
        }
    }
?>
                        <div class="report_table_body" style="width:100%; margin:0 auto; padding:10px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block;">
                            ...
                            <div style="width:8%;margin-left:16%">
                                <span style="float:left; width:100%; text-align:center"><?php echo $total_inventory ?></span>
                            </div>
                            <div class="kucun_sp" style="width:18%;margin-left:5%;text-align:center;"><?php echo $kucun ?></div>
                            <div style="width:6%;margin-left:7%"><?php echo round($avg_num) ?></div>
