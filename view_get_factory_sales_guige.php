...
$where=array(
    "detail_boss_m_bianhao=? and detail_master_bianhao=? and detail_p_huohao=? and detail_p_color!='' and detail_order_type in ('". join("','",$types) ."') and detail_factory_bianhao=?",
    $boss_id,$order_master_id,$_REQUEST["get_sales_guige_huohao"],$factory_id
);
...
?>
                        <div style="width:100%; margin:0 auto; padding:10px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block;">
                            ...
                            <div style="float:left; width:15%; text-align:center"><?php echo $row_main["detail_p_color"] ?></div>
                            <div style="float:left; width:15%; height:15px"></div>
                            <div style="float:left; width:15%; text-align:center"><?php echo $row_main["pool"] ?></div>
                            <div style="float:left; width:10%; text-align:center"><span style="font-size:14px; color:#0099FF; cursor:pointer" onclick="/**/ShowSalesProductSize('<?php echo $_REQUEST["get_sales_guige_huohao"] ?>','<?php echo $row_main["detail_p_color"] ?>','<?php echo date("Y-m-d",$row_main["detail_order_{$ymd}"]) ?>')">详细</span></div>
                        </div>
...
function ShowSalesProductSize(p_huohao,p_color,time)
{
    if ($("#layer_sales_product_guige_"+p_huohao+"_"+p_color+"_"+time).is(":visible")==false)
    {
        ...
        $.ajax({
            url:"view-get-factory-sales-size", 
            async: false,
            type: "POST",
