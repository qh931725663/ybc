...
?>
                <div style=" float:left; width:98%; margin-top:0px; padding:15px 1%; background:#ffffff; overflow:hidden; display:block">
                    <div id="step_main" style="width:100%; overflow:hidden; display:block">
                        <div style="float:left; width:40%; margin-top:0px; overflow:hidden; display:block;">
                            <div id="pickuporderlist_storewarehouse_name" style="float:left; margin-left:11px"></div>
                            <div id="pickuporderlist_sales_store_name" style="float:left; margin-left:35px"></div>

                        </div>
                        <div style="position:relative; float:left; width:100%; margin:-25px 0 10px; overflow:hidden; display:block;">
                            <div style="float:left; margin-left:9px; margin-top:37px">
                                当前卖家：<span id="pickuporderlist_seller_name" class="isellername" style="font-size:24px; font-weight:bold; color:#ee583d"></span><span id="pickup_note_seller_cycle" style="margin-left:10px; font-size:24px; font-weight:bold; color:#ee583d"></span>
                            </div>
                            <div style="float:right; margin-right:125px;">
                                ...
                                <span id="pickup_notice_count" class="inoticecount" style="float:left; margin-left:10px; padding:5px 50px; font-size:48px; font-weight:bold; color:#ffffff; background:#d64126">0</span>
                            </div>
                            <div style="float:right">
                                <span style="position:absolute; top:4px; right:0;" class="block_btn_normal_red" onclick="/**/mount_to_frame('view_sales_pickup',1,'frame_sales_pickup')">放弃返回</span>
                            </div>
                        </div>
                        <div style="width:98%; margin:0 auto; background:#ffffff; border:1px solid #cccccc; overflow:hidden; display:block">
                            <div id="pickuporderlist_left" style="float:left; width:56%; min-height:420px; padding:0 2%; border-right:1px solid #cccccc; overflow:hidden; display:block">

                                <div style="float:left; width:98%; padding:10px 1%">
                                    <span style="float:left">已选择商品列表</span>
                                    <span style="float:right">
                                        <span style="font-size:18px; color:#999999">数量总计：<span id="pickup_total_num" class="itotalnum" style=" font-size:24px; color:#ee583d">0</span>, 金额总计：<span id="pickup_total_money" class="itotalmoney" style="font-size:24px; color:#ee583d">0</span></span>
                                    </span>
                                </div>
                                <form method="post" id="pickup_order_form">
                                <div style="max-height:280px; overflow-y:auto; float:left; width:98%; padding:5px 1%; display:block">
                                ...
                                <div style="width:98%; margin:2px auto 0 auto; overflow:hidden; display:block">
                                    <div style="float:right; width:150px; overflow:hidden; display:block">
                                        <div id="btn_pickup_putintoorderlist" class="btn_putintoorderlist" style="margin:3px 0;">确认提交出库</div>
                                    </div>
                                </div>
                            ...
                            <div id="pickuporderlist_right" style="float:left; width:35%; padding:0 2%; overflow:hidden; display:block">
                                ...
                                <div style="float:left; width:98%; padding:5px 1%">
                                    <div style="position: relative; width:100%; margin:5px 0; padding:5px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block">
                                        <div class="ikeyboard" onclick_before="view_sales_pickup_submit.click_keyboard"></div>
...
function get_pickup_barcode_str(info)
{
    ...
    {    
        price_input_html="<input name='table["+order_pickup_idx+"][order_price]' type='text' maxlength='100' style='width:50px; padding:2px; text-align:center; background:#dddddd; border:0; color:#ee583d' value='" +info["price"]+ "' class='order_price iedit_price' onKeyUp='/**/setPickupListPriceModify($(this))'>";
    }
    else
    {
        price_input_html="<input name='table["+order_pickup_idx+"][order_price]' type='text' maxlength='100' style='width:50px; padding:2px; text-align:center; background:#ffffff; border:0; color:#ee583d' value='" +info["price"]+ "' class='order_price iedit_price' onKeyUp='/**/setPickupListPriceModify($(this))' readonly>";
    }
...
function GetPickupHuohaoByFirstchar(obj, select_pickup_firstchar)
{
    ...
    $("#select_pickup_huohao").hide();//暂时隐藏
...
function GetPickupFactoryByHuohao(obj, select_pickup_huohao)
{
    ...
    {    
        ...
        var_factory_html+="<div class='ifactory' style='float:left; width:25%; padding:5px 0; color:#999999; text-align:right; overflow:hidden; display:none;'>工厂：</div>";
        ...
        if($(".select_pickup_factory_radioToggle").css("display")=="block"){//同步厂家显示
            $(".ifactory").show();
        }
...
var letters="";
for(var keys in pickup_gethuohaobyfirstchar){
    letters+=keys;
}
...
function click_keyboard(pid)
{
    ...
    /**/GetPickupFactoryByHuohao(null, pid);
}
</script>
