...
$where=array(
    "detail_boss_m_bianhao=? and detail_master_bianhao=? and detail_master_bianhao!=0 and detail_p_huohao=? and detail_p_color!='' and detail_factory_bianhao=?",
    $boss_id,$order_master_id,$_REQUEST["get_stock_guige_huohao"],$factory_id
);
$sums=array();
...
function ShowStockProductSize(p_huohao,p_color,time)
{
    if ($("#layer_stock_product_guige_"+p_huohao+"_"+p_color+"_"+time).is(":visible")==false)
    {
        ...
        $.ajax({
            url:"view-get-factory-stock-size", 
            async: false,
            type: "POST",
