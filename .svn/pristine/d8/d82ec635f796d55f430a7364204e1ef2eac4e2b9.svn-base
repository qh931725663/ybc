...
try
{
    ...
    $experience_account=!empty($_REQUEST["var_experience_account"])?$_REQUEST["var_experience_account"]:"0";
    
    if ($experience_account=="10000024" or $experience_account=="10000025")
    {
        $p=cselect("*","ydf_member",array("m_bianhao=?",$experience_account));
	}
    else
    {
        $p=cselect("*","ydf_member",array("(m_mobile=? or m_bd_mobile=?) and m_password=?",$_REQUEST["var_mobile"],$_REQUEST["var_mobile"],md5($_REQUEST["var_password"])));
    }
    if ($rowcheck=$p[0]->fetch())
	{
		...
		if ($rowcheck["m_type"]=="4")
		{
			...
			{
				$p_store=cselect("*","ydf_dangkou",array("dangkou_type='1' and dangkou_boss_m_bianhao=?",$rowuser["user_boss_m_bianhao"]));
				...
				if ($countdangkou=="1" and $countwarehouse=="0")
				{		
					$_SESSION["ERP_ACCOUNT_USER_DANGKOU_COUNT"]="1";
        ...
        elseif ($rowcheck["m_type"]=="3")
        {
            throw new PDOException("卖家用户请在手机应用中登录使用！");
        }
	}
	else
	{
        if ($experience_account<>"10000016" and $experience_account<>"10000024")
        {
