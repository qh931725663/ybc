...
function send_mobilebd_code($appkey,$secret,$mobile)
{
    ...
    $req ->setSmsParam( "{code:'$code',product:'【优百仓档口管家】'}" );
    $req ->setRecNum( $mobile );
    $req ->setSmsTemplateCode( "SMS_10855218" );
    $resp = $c ->execute( $req );
    return $resp;
}
