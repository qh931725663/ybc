...
{    
    ...
    $('#chuku_order_is_cashier').attr("value","");
    $('#chuku_order_is_cycle').attr("value","");
    $('#chuku_order_ckph_sdsy').attr("value","");

    if (state=="temp")
        $('#chuku_order_temp_sdsy').attr("value","1");
    if (state=="havepay")//已付款订单
    {
        $('#chuku_order_pay_sdsy').attr("value","1");
        $('#chuku_order_is_cashier').attr("value","0");
    }
    if (state=="pay")//未付款订单
    {
    ...
    {
        $('#chuku_order_is_cashier').attr("value","");
        $('#chuku_order_is_cycle').attr("value","0");
    }
    if (state=="ckph")
...
<div class="sdsy_setup">
    ...
    <div class="sdsy_nave" style="margin-top:20px;">
        <div class="sdsy_nave_a">
            ...
            <input type="hidden" id="chuku_order_is_cashier" name="chuku_order_is_cashier" value=""/>
            <input type="hidden" id="chuku_order_is_cycle" name="chuku_order_is_cycle" value=""/>
            ...
            <div style="display:inline-block; padding-top:2px;" id="paixv_box">
                <span style="float:left; margin-top:2px;"><img src="/pc/images/39-Clock.png"></span>
                <span id="update_time" style="float:right; margin-left:5px; margin-top:3px; text-align:left; width:90px;" class="clear_search">按更新时间</span>
                <span id="creation_time" style="float:right; margin-left:5px; margin-top:3px; text-align:left; width:90px; display:none" class="clear_search" >按创建时间</span>
            </div>
        </div>
        <div class="sdsy_nave_b">
            ...
            <input id="order_by" type="hidden" value="1" name="order_by">
        </div>
    </div>                        
</div>
</form> 
...
if(@$chuku_searchwords=="匿名卖家"){
    $where=@array("order_boss_m_bianhao=? and (order_master_bianhao=? or order_slave_bianhao=?) and order_is_pay=? and order_seller_cycle=? and order_seller_cycle>? and order_addtime>=? and order_addtime<=? and order_type in ${order_type} and order_seller_bianhao='1'",
            $boss_id,$order_master_id,$order_master_id,$_REQUEST["order_is_pay"],$chuku_order_is_cashier,$chuku_order_is_cycle,$from_day,$to_day);
}else{
    $where=@array("order_boss_m_bianhao=? and (order_master_bianhao=? or order_slave_bianhao=?) and order_is_pay=? and order_seller_cycle=? and order_seller_cycle>? and order_addtime>=? and order_addtime<=? and order_type in ${order_type} and order_seller_name=?",
            $boss_id,$order_master_id,$order_master_id,$_REQUEST["order_is_pay"],$_REQUEST["chuku_order_is_cashier"],$_REQUEST["chuku_order_is_cycle"],$from_day,$to_day,$chuku_searchwords);
}
    ...
    <div class="order_inner">
		<div class="order_table_header">
            ...
            <div style="float:right; text-align:right">
                <?php if ($roworder["order_type"]=="ckph" and $roworder["order_is_pickup"]==0 and $roworder["order_seller_cycle"]>0){ ?>
                <span class="btn_order_red" style="color:#fff" onclick="/**/ShowSetChukuOrderPickupStatus(<?php echo $roworder["order_bianhao"]?>)">修改提货状态</span>
                <?php } ?>
                <?php if ($roworder["order_type"]<>"xsckgq" and $roworder["order_seller_cycle"]==0 and ((($_SESSION["ERP_ACCOUNT_LOGIN_TYPE"]=="1"  or (!empty($_SESSION["ERP_ACCOUNT_USER_TYPE"]) and $_SESSION["ERP_ACCOUNT_USER_TYPE"]=="1")) and empty($_SESSION["ERP_ACCOUNT_USER_DANGKOU_TYPE"])) or (!empty($_SESSION["ERP_ACCOUNT_USER_DANGKOU_TYPE"]) and $_SESSION["ERP_ACCOUNT_USER_DANGKOU_TYPE"]=="1"))){ ?>
...
function ShowSetChukuOrderPickupStatus(order_bianhao){
    $.ajax({
...
function PostSetChukuOrderPickupStatus(){
    $.ajax({
        url:"model-set-chukuorderpaystatus", 
        async: false,
        type: "POST",
        data:$("#form_order_set_pickupstatus").serialize(),
        success: function(html){
