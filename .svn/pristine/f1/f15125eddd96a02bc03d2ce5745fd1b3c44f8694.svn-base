...
function clean_search_stock_bi()
{
	mount_to_frame('view_stock_bi_stock_time',1,'frame_stock_bi_stock_time');
						...
						<div style="float:left; width:100%; overflow:hidden; display:block">
							<div  style="float:left; width:100%; margin:10px 0; padding:5px; overflow:hidden; display:block">
								...
								<div style="float:right; overflow:hidden; display:block">
									<span style="float:left"><input id="search_stock_bi_huohao" class="iinput name_iime ino_ime_input iproduct_name" name="search_stock_bi_huohao" type="text" style="width:150px; padding:4px 3px 3px 4px; font-size:14px;" placeholder="请输入商品货号"/></span>
						...
						<div style="width:100%; margin:0 auto; padding:10px 0; border-bottom:1px solid #cccccc; overflow:hidden; display:block;">
							...
							<div style="float:left; width:10%; color:#999999; text-align:center;margin-left:8%">库存</div>
							<div style="float:left; width:10%; color:#999999; text-align:center;margin-left:8%">七日总销</div>
							<div style="float:left; width:10%; color:#999999; text-align:center;margin-left:8%">七日均销</div>
...
for ($i=$offset;$i<$offset+$pagesize && $i<$rowcount;$i++)
{
    ...
    if(array_key_exists($row_main["detail_p_huohao"],$event_inventory)==false){
        $avg_num=0;
        $total_num=0;
    }else{
        $avg_num=round($event_inventory[$row_main["detail_p_huohao"]]["avg_num"]);
        $total_num=round($event_inventory[$row_main["detail_p_huohao"]]["total_num"]);
    }
?>
						<div style="width:100%; margin:0 auto; padding:10px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block;">
							...
							<div style="float:left; width:10%; text-align:center;margin-left:8%"><?php echo round($row_main["sum"]["pool"]) ?></div>
							<div style="float:left; width:10%; height:15px;text-align:center;margin-left:8%"><?php echo $total_num ?></div>
							<div style="float:left; width:10%; height:15px;text-align:center;margin-left:8%"><?php echo $avg_num ?></div>
							<div style="float:left; width:10%; text-align:center；display:none;"><span style="color:#0099FF; cursor:pointer;display:none;" onclick="/**/ShowStockProductGuige('<?php echo $row_main["detail_p_huohao"] ?>','<?php echo date("Y-m-d",$row_main["detail_order_{$ymd}"]) ?>')">详细</span></div>
						</div>
