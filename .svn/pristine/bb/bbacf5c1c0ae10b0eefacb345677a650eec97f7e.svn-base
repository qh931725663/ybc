...
list($historys2,$addup)=get_history_stock_num($ymd="day",$group,$factory_id,$product_id,null,null,null,null);
debug($addup);
...
for ($i=0;$i<$rowcount;$i++)
{
    ...
    for($p=0;$p<count($dangkou_arr);$p++){
        ...
        $huo_dang=$dangkou_arr[$p][0];
        $dangkou_name=$dangkou_arr[$p][1];
        if(array_key_exists($huohao.'#'.$color.'#'.$huo_dang,$detail_addup)==false){
                $kucun_x1.="<span class='lf' style='margin-top:4px;margin-right:5px;'>".$dangkou_name.":0"."</span>";
        }else{
            $kucun_x1.="<span class='lf' style='margin-top:4px;margin-right:5px;'>".$dangkou_name.":".floatval($detail_addup[$huohao.'#'.$color.'#'.$huo_dang]['pool'])."</span>";
        }
    }
if($color!=""){
?>
                        ...
                        <div style="width:100%; margin:0 auto;background:#f2f2f2; padding:10px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block;">
                            ...
                            <div class="kucun_sp" style="float:left; width:17%;text-align:center;margin-left:6%"><?php echo $kucun_x1 ?></div>
                            <div style="float:left; width:8%;text-align:center;margin-left:7%;"><?php echo $avg_num ?></div>
