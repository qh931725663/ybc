...
list($historys2,$addup)=get_history_stock_num($ymd="day",$group,$factory_id,$product_id,$order_master_id,$kucun_color,null,null);
debug($addup);
//七日总销和均销
$group=array("detail_p_huohao","detail_p_color","detail_p_size");
...
?>
    <div style="width:100%;background:#f2f2f2; margin:0 auto; padding:10px 0; border-bottom:1px dashed #cccccc; overflow:hidden; display:block;">
        ...
        <div style="float:left; width:6%; height:15px;text-align:center;margin-left:15%"><?php echo $avg_num ?></div>
        <div style="float:left;width:6%;height:14px;text-align:center;margin-left:15%"><?php echo $total_num ?></div>
    </div>
<?php
}
?>